...
1.  **克隆仓库**:
    ...
    ```bash
    git clone https://github.com/Jimeng-AI/ComfyUI-Jimeng-API.git
    ```
...
- **智能统一节点**:
  - **图像生成**:
    ...
    - `即梦 Seedream 4` 节点支持更高级的图生图（最多 10 张输入图像）和图组生成模式。
...
- **视频生成**:
  - `即梦视频生成`: **统一的核心视频生成节点**。支持**文生视频**、**图生视频**（首帧）和**首尾帧生视频**三种模式。
  - `即梦参考图转视频`: 根据一张或多张**参考图像**生成视频。
