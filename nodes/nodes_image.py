...
class JimengImageHelper:
    ...
    def validate_custom_size(
        self, width, height, min_pixels, max_pixels
    ):
        """
    ...
    async def execute_generation(self, generation_count, generate_single_func, ignore_errors=False):
        """
        执行批量生成任务。
        ...
        return self.process_batch_results(results, generation_count, ignore_errors)

    def process_batch_results(self, results, generation_count, ignore_errors=False):
        """
        处理批量生成的结果。
        ...
        if not valid_tensors:
            if ignore_errors:
                ...
                dummy_image = create_white_image_tensor(1024, 1024)
                return comfy_io.NodeOutput(dummy_image, json.dumps({"error": "All tasks failed but ignored. Returning dummy image."}))
...
class JimengSeedream3(comfy_io.ComfyNode):
    ...
    async def execute(
        ...
        result = await helper.execute_generation(generation_count, _generate_single, ignore_errors=ignore_errors)
...
class JimengSeedream5(comfy_io.ComfyNode):
    ...
    async def execute(
        ...
        async def _generate_single(idx, session):
            ...
            return await helper.stream_generation(
