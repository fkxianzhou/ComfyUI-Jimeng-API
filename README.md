...
- **智能统一节点**:
  - **图像生成**:
    ...
    - `即梦 Seedream 4` 节点支持更高级的图生图（最多 14 张输入图像）和组图生成模式。
...
支持最新的即梦 Seedream 4.0 及 4.5 模型，提供更高质量的图像生成能力，并支持多图参考和组图生成功能。

* **Model Version (模型版本)**:
    * `doubao-seedream-4.5`: 最新模型，效果更佳。
    * `doubao-seedream-4.0`: 标准模型。
* **Enable Group Generation (启用组图生成)**:
    * **`True` (开启/Auto)**: 激活**组图模式**。生成一组内容关联的图片。
        * **支持场景**: 文生组图、单图生组图、多图生组图。
        * **数量限制**: `输入参考图数量` + `生成图片数 (max_images)` ≤ **15**。
    * **`False` (关闭/Single)**: 激活**单图模式**。
        * **支持场景**: 文生图、单图生图、多图生图。
        * **注意**: 此模式下无论输入多少张参考图，最终仅生成 **1** 张图片。
* **Prompt**: 文本提示词。
* **Max Images**: (仅在组图模式下生效) 设置生成的图片数量。
* **Images (Optional)**: 输入参考图像，支持单张或多张（最多 14 张）。
    * 连接 1 张图片 -> 单图参考。
    * 连接多张图片 (Batch) -> 多图参考。

> **注意**: 在组图模式下，请务必确保 `输入图片数 + 生成图片数` 不超过 15，否则会报错。
